...
- WO snapshot: `mrp_workorder.activity_snapshot` (reserved field in `schema.skeleton.prisma`) — copy routing activity state at work-order creation time so historical WOs are unaffected by template changes
...
Too rigid. Steel fabrication has legitimate per-product deviations (custom profiles, non-standard weld lengths) that cannot be encoded purely in formula parameters. Forcing every deviation into a formula parameter would make the formula engine too complex.

### Option 1 — Per-product clone (rejected for large catalog)
Acceptable for < 500 products but fails at 50,000+. Template updates require mass re-cloning. No diff visibility.
