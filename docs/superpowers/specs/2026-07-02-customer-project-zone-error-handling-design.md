...
Third feature of the repo-wide error-handling initiative (Login, then BOM, both shipped to `main`). This pass covers `CustomerList.tsx`, `ProjectList.tsx`, `ZoneList.tsx` — 6 mutations that currently have **zero** error handling, plus one existing handler using an old ad-hoc pattern.
...
**Goals**
- Guard all 6 currently-unhandled mutations so a failure doesn't silently continue into success-path code (closing modals, resetting forms, navigating) and doesn't fail silently for the user.
...
- **Activate the mutation opt-in mechanism for the first time.** The global `MutationCache.onError` handler (built during BOM, opt-in via `meta.showGlobalErrorToast`) has had zero call sites using it since it shipped — every one of these 6 mutations is exactly the case it was designed for (new/unhandled mutation, no existing local error toast to conflict with).
...
Add `meta: { showGlobalErrorToast: true }` as a sibling property to the 6 mutation definitions:
...
- `useSubZones.ts`: `useCreateSubZone`
...
| Any of the 6, on success | Local `toast.success(...)` | Toast + existing success-path UI (close modal / navigate / etc.) runs |
...
6. Full regression with backend running: all 6 actions succeed normally, each shows its success toast, existing `handleDelete` behavior unchanged (still works, just uses the shared helper now).
