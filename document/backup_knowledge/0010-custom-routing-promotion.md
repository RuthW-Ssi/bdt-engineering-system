...
3. **FE suggestion banner** — `CustomRoutingEditor` polls promotion candidates and shows a green banner when the current product belongs to a group with `count ≥ 3`.

## Similarity algorithm

Two custom routings are considered structurally similar if their sorted op_code arrays produce the same join key. Activity-level matching is not required at promotion time.
...
- Promotion does not delete the source `custom_routing` record; it becomes orphaned and can be cleaned up in a future maintenance sprint.
